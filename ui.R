...
ui <- fluidPage(theme = shinytheme("slate"),
  # Title
      navbarPage(title = div(img(src = "logo.png", height = 60, width = 40, style = "padding-bottom:10px"), 
        ...
        sidebarPanel(
          ...
          wellPanel(
            sliderInput("Number", label = "Choose the Top .. you want to see:", 1, 10, value = 5))
  ...
  sidebarPanel(
    ...
    # Sidebar
    wellPanel(
    selectInput("Region", h5("Select a Country:"), sort(unique(dt.spotify$Region)),
                selected = "Argentina"),
    dateInput("date.selector", h5("Select a Date:"),  min = "2017-01-01", max = "2017-12-31", value = "2017-01-01", startview = "month")
    )),
  
  mainPanel    (   
    titlePanel("Spotify Top 10 by Day by Country!"),
    p("Use the filters to see the top 10 most listened songs on Spotify in a specified country and on a specified day."),
       ...
       DT::dataTableOutput("country")
        ...
        tabPanel("Countries", htmlOutput("basic.network"),
          ...
          sidebarPanel(
            wellPanel(
            selectInput("country.selector", "Select a Country:", sort(unique(dt.spotify$Region)),
                        selected="Argentina"),
            ...
            checkboxGroupInput("centrality_measures", label = h3("Calculate centrality measures:"), 
        ...
        mainPanel(
          ...
          p("Below you can find a map with a network of your selected country. The countries are connected if those countries listen to the same artists. On the left you can select how many artists the countries need to have in common to be connected. Moreover, you can select the time period over which the app calculates the similarities. Furthermore, you can filter on the top number of songs. So for example the app can only look at the top 3 songs in each country and find the connections. "),
            leafletOutput("network.map", width = 1000, height = 600),
    ...
    tabPanel("Descriptives", htmlOutput("descriptives"),
      sidebarPanel(
        wellPanel(
        selectInput("country.selector.2", "Select a Country:", sort(unique(dt.spotify$Region)),
                    selected="Argentina"),
